      ...
      <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div class="relative my-4">
            ...
            <div class="relative flex justify-center text-xs"><span class="px-2 bg-gray-50 text-gray-500">or</span>
            </div>
          </div>
          ...
          <p class="text-center text-xs text-gray-600 pt-2">
            ...
            <router-link to="/signup" class="text-blue-600 hover:text-blue-700 font-semibold transition">
              Sign Up
            </router-link>
          </p>
        </form>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    alertToast.value?.success('Welcome back!', 'You are now signed in')
    router.push('/')
  } catch (error) {
    switch (error.code) {
      ...
      case 'auth/user-not-found':
        ...
        alertToast.value?.error('Login failed', 'Invalid email or password')
        break
...
const handleGoogleLogin = async () => {
  try {
    ...
    alertToast.value?.success('Google login successful!', 'Welcome back')
    router.push('/')
  } catch (error) {
    errorMessage.value = 'Could not initiate Google login'
    alertToast.value?.error('Google login failed', error.message)
  }
}
</script>
